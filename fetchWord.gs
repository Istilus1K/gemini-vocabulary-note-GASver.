...
const prompt = `${language} の「${word}」という単語またはフレーズを辞書的に解説してください。\n\n` +
  ...
  "3. 意味 (meaning)（必ず日本語で、品詞情報を先頭に付けて書く。例：【名詞】心）\n" +
  "4. 例文1 (example1)\n" +
  ...
  "### 追加ルール\n" +
  "- 出力は必ず9行とし、すべての項目を改行で区切ること。\n" +
  "- 例文は**簡潔(文字数が多くない)で、**自然な文脈で使用されるものにすること。\n" + // ← 変更点
  "- 各項目について情報が見つからない場合は、その行に「情報が不足しています」と記載すること。\n" +
  "- 入力された単語やフレーズ自体が見つからない場合は、他の情報を記載せず、「結果が見つかりませんでした。」とだけ出力すること。\n\n" +
